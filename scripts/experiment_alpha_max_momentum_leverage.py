"""Alpha-Max 統合システム: モメンタム集中投資 ＆ 動的信用レバレッジ ＆ トレイリング防御バックテストスクリプト

動的モメンタム・スクリーニング、マクロ連動の動的信用レバレッジ、ブレークイーブン・トレイリングストップの効果を検証する。
"""
...
class AlphaMaxPortfolioSimulator:
    def __init__(
        self,
        initial_cash: float = 1_000_000.0,
        max_positions: int = 3,                       # 集中投資: 最大2〜3銘柄
        target_leverage: float = 2.5,                 # 強気時最大レバレッジ (2.0〜3.0倍)
        leverage_mode: str = "adaptive_macro",        # "fixed" or "adaptive_macro"
        use_trailing_protection: bool = True,         # ブレークイーブン・トレイリング防御
        ...
        self.use_trailing_protection = use_trailing_protection
    ...
    def get_active_leverage(self, vix: float, sox_ret: float) -> float:
        ...
        if vix > 21.0 or sox_ret < -0.025:
            ...
            return 1.0  # 警戒時: 現物等倍 1.0倍
        elif vix < 16.0 and sox_ret > 0.0:
            return self.target_leverage  # 強気時: 最大レバレッジ (2.5〜3.0倍)
        else:
    ...
    def step(
        ...
        active_leverage = self.get_active_leverage(vix, sox_ret)

        # 1. 決済チェック (3日スイング ＆ トレイリング・SL判定)
        closed_tickers = []
        for ticker, pos in list(self.positions.items()):
            ...
            # トレイリング防御: 一度 +1.5 ATR 以上の高値をつけたら、SLを買値 (ブレークイーブン) に引き上げ
...
def run_alpha_max_experiments():
    ...
    for code, df_stock in stock_data_dict.items():
        ...
        # 高度モメンタム複合スコア (大口サージ * ボラティリティ拡大 * 高値ブレイク)
    ...
    # Walk-Forward 3年ローリング推論 (2024〜2026年)
    ...
    # 検証シナリオ
    scenarios = [
        # (max_pos, target_lev, lev_mode, trailing, label)
        (5, 1.0, "fixed", False, "① 【基準】現物 1.0倍 (5分散 × 3日保有)"),
        (2, 1.0, "fixed", False, "② 【モメンタム集中】現物 1.0倍 (Top2集中 × 3日保有)"),
        (5, 2.5, "adaptive_macro", False, "③ 【動的レバレッジ】5分散 × 最大2.5倍 (益出し複利)"),
        (2, 2.0, "adaptive_macro", True,  "④ 【Alpha-Max 2.0倍】Top2モメンタム集中 × レバレッジ2.0倍 × トレイリング防御"),
        (2, 2.5, "adaptive_macro", True,  "⑤ 【Alpha-Max 2.5倍】Top2モメンタム集中 × レバレッジ2.5倍 × トレイリング防御"),
        (2, 3.0, "adaptive_macro", True,  "⑥ 【Alpha-Max 3.0倍】Top2モメンタム集中 × レバレッジ3.0倍 × トレイリング防御"),
    ]
    ...
    for max_pos, lev, mode, trailing, label in scenarios:
        sim = AlphaMaxPortfolioSimulator(
    ...
    print("\n==========================================================================================================")
    print("   ALPHA-MAX STRATEGY EXPERIMENT RESULTS (2024 - 2026 : 2.5 YEARS OUT-OF-SAMPLE)")
    print("==========================================================================================================")
    ...
    for d in sample_dates:
        row = {"Date": d}
        for lbl in [scenarios[0][4], scenarios[1][4], scenarios[2][4], scenarios[4][4], scenarios[5][4]]:
