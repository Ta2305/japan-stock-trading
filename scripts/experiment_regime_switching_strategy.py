"""マクロ・レジームスイッチング (Macro Regime Switching) 全天候型トレード戦略バックテストスクリプト

市場レジームに応じてロング/ショートを切り替える戦略の成績を、ML有無・資金管理方式ごとに比較する。
"""
...
class RegimeSwitchingSimulator:
    def __init__(
        ...
        position_sizing_mode: str = "fixed_20pct",  # "fixed_20pct" or "unconstrained_top"
        holding_mode: str = "fixed_3days",           # "fixed_3days" or "unlimited_trend"
        emergency_cash_flush: bool = False,         # 下落レジーム突入時に既存Longを即座に手仕舞うか
        slippage_fee_pct: float = 0.001,
        stock_borrowing_rate_daily: float = 0.00003,
        ...
        self.emergency_cash_flush = emergency_cash_flush
    ...
    def step(
        ...
        for ticker, pos in list(self.positions.items()):
            ...
            # 緊急避難チェック (Bearレジーム突入時に既存Longを即座に損切り/利確決済)
            if self.emergency_cash_flush and regime == "BEAR" and side == "long":
                exit_triggered = True
                exit_reason = "Emergency Cash Flush (Regime Shift to Bear)"
                exit_price = p_data["open"] * (1.0 - self.slippage_fee_pct)

            elif self.holding_mode == "fixed_3days":
                if pos["days_held"] >= 3:
                    exit_triggered = True
        ...
        # 2. 新規エントリー執行 (レジームに基づく厳格なフィルタリング)
        total_equity = self.get_total_equity(current_prices)

        if regime == "BULL":
            # 強気相場: 買いのみ
            entries = [(t, "long") for t in buy_signals if t not in self.positions and t in daily_prices]
        else:
            # 危機・下落相場: 空売りのみ
            entries = [(t, "short") for t in short_signals if t not in self.positions and t in daily_prices]
...
def run_regime_switching_experiments():
    ...
    for code, df_stock in stock_data_dict.items():
        ...
        # レジーム判定 (マクロ基準)
        # BEAR: VIX > 21 または SOX前日騰落率 < -1.5%
        is_bear = (merged["vix_close"] > 21.0) | (merged["sox_return_1d"] < -0.015)
        merged["market_regime"] = np.where(is_bear, "BEAR", "BULL")
    ...
    for _, row in test_df.iterrows():
        d = row["date"]
        t = row["ticker"]
        regime_by_date[d] = row["market_regime"]
    ...
    scenarios = [
        # (use_switching, use_ml, sizing_mode, holding_mode, flush, label)
        (False, True,  "fixed_20pct", "fixed_3days", False, "① [MLあり] 買いのみ (Long Only) 5分散 × 3日保有 (従来)"),
        (True,  True,  "fixed_20pct", "fixed_3days", False, "② [MLあり] レジームスイッチング 5分散 × 3日保有"),
        (True,  True,  "unconstrained_top", "fixed_3days", False, "③ [MLあり] レジームスイッチング Top集中 × 3日保有"),
        (True,  True,  "fixed_20pct", "fixed_3days", True,  "④ [MLあり] レジームスイッチング + 暴落時緊急避難 5分散 × 3日保有"),
        (True,  True,  "unconstrained_top", "fixed_3days", True,  "⑤ [MLあり] レジームスイッチング + 暴落時緊急避難 Top集中 × 3日保有"),
        (False, False, "fixed_20pct", "fixed_3days", False, "⑥ [MLなし] 買いのみ (Long Only) 5分散 × 3日保有 (純テクニカル)"),
        (True,  False, "fixed_20pct", "fixed_3days", False, "⑦ [MLなし] レジームスイッチング 5分散 × 3日保有"),
        (True,  False, "unconstrained_top", "fixed_3days", False, "⑧ [MLなし] レジームスイッチング Top集中 × 3日保有"),
    ]

    results = []
    equity_curves_dict = {}

    for use_switch, use_ml, sizing, holding, flush, label in scenarios:
        sim = RegimeSwitchingSimulator(
            initial_cash=1_000_000.0,
            position_sizing_mode=sizing,
            holding_mode=holding,
            emergency_cash_flush=flush,
        ...
        for d in test_dates:
            ...
            current_regime = regime_by_date.get(d, "BULL") if use_switch else "BULL"
            
            sim.step(
                date=d,
                regime=current_regime,
                daily_prices=daily_stocks,
                buy_signals=pending_long,
                short_signals=pending_short,
            )
            ...
            pending_short = [c[0] for c in c_short[:5]] if use_switch else []
    ...
    print("\n======================================================================================================================")
    print("   REGIME SWITCHING COMPREHENSIVE BACKTEST RESULTS (2026 OUT-OF-SAMPLE)")
    print("======================================================================================================================")
    print(pd.DataFrame(results).to_string(index=False))
    print("======================================================================================================================\n")

    # 7月 (急落月) における資産推移比較
    print("=== [2026年7月 (SOX急落・調整局面) における資産残高推移] ===")
    sample_dates = ["2026-06-30", "2026-07-15", "2026-07-31", "2026-08-04"]
    tracking_rows = []
    for d in sample_dates:
        row = {"Date": d}
        for lbl in [scenarios[0][5], scenarios[1][5], scenarios[2][5], scenarios[3][5], scenarios[4][5]]:
